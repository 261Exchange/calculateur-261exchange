import streamlit as st import datetime import pandas as pd import requests import matplotlib.pyplot as plt from io import BytesIO
...
st.title("💱 261 Exchange – Calculateur Pro") st.write("Calcule rapidement le montant à envoyer ou à recevoir selon le taux, les frais et le sens de conversion.")
...
st.subheader("📊 Cours actuels des cryptomonnaies") for symbol, coingecko_id in crypto_ids.items(): usd_price = crypto_prices[coingecko_id]["usd"] ar_depot = usd_price * 4850 ar_retrait = usd_price * 4300 st.markdown(f"{symbol}: {usd_price:.6f} USD | Dépôt: {ar_depot:,.0f} Ar | Retrait: {ar_retrait:,.0f} Ar")
...
montant_ariary = 0 montant_usd = 0

if sens == "🔁 Ariary ➜ USD": montant_ariary = st.number_input("Montant payé (en Ariary)", min_value=0.0, step=1000.0) else: montant_usd = st.number_input("Montant à envoyer (en USD)", min_value=0.0, step=0.01)
...
if sens == "🔁 Ariary ➜ USD": montant_usd_brut = montant_ariary / taux montant_final = montant_usd_brut - frais else: montant_ariary = (montant_usd + frais) * taux montant_final = montant_usd

Résultat utilisateur

st.markdown("### 💡 Résultat") st.write(f"📤 Montant à envoyer : {montant_final:.2f} USD") st.write(f"🔸 Frais appliqués : {frais:.2f} USD") if sens == "🔁 USD ➜ Ariary": st.write(f"💵 Montant à recevoir : {montant_ariary:.0f} Ar")

Historique

now = datetime.datetime.now().strftime("%d/%m/%Y %H:%M") st.session_state.historique.append({ "Date": now, "Opération": operation, "Service": service, "Montant MGA": f"{montant_ariary:.0f} Ar", "Montant USD": f"{montant_final:.2f} USD", "Frais": f"{frais:.2f} USD" })

Copier et exporter

if st.button("📋 Copier le résultat"): st.code(f"{montant_final:.2f} USD | {montant_ariary:.0f} Ar", language='text')

df = pd.DataFrame(st.session_state.historique) st.download_button("⬇️ Exporter CSV", data=df.to_csv().encode(), file_name="historique_261_exchange.csv", mime="text/csv")
...
st.markdown("---") admin_pw = st.text_input("🔒 Mot de passe admin", type="password") if admin_pw == "admin261exchange": st.success("✅ Accès administrateur autorisé") st.subheader("📜 Historique complet de session") st.dataframe(df) else: st.info("Seul l’administrateur peut consulter l’historique complet.")
